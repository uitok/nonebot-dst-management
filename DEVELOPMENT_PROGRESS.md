...
#### 2.1 AI 诊断模式增强 🔥 新增
- [ ] **AI 模组配置诊断**（从"自动优化"改为"问题诊断"）
  - [ ] 修改 AI Prompt，输出结构化问题清单
  - [ ] 问题分级：critical/warning/info
  - [ ] 问题类型：missing/conflict/invalid/performance
  - [ ] 每个问题包含：影响说明 + 修复建议 + 修改理由
- [ ] **新增命令：/dst mod config show** - 查看完整优化配置
- [ ] **新增命令：/dst mod config apply** - 应用 AI 建议的配置
  - [ ] 支持 `--auto` 参数（自动应用所有建议）
  - [ ] 支持 `--dry-run` 参数（预览更改）
  - [ ] 交互式确认模式（逐项确认）
- [ ] **AI 存档诊断增强**
